...
const canonical = "https://garden-kimtaehoon-office.vercel.app/contact";
...
export default function ContactPage() {
  const breadcrumb = { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
    { "@type": "ListItem", position: 1, name: "홈", item: "https://garden-kimtaehoon-office.vercel.app/" },
    { "@type": "ListItem", position: 2, name: "상담 신청", item: canonical },
  ] };
  return <>
    ...
    <section className="contact-process-section"><div className="shell">
      ...
      <ol className="contact-process">{process.map(([Icon, label], index) => <li key={label}><span>{String(index + 1).padStart(2, "0")}</span><Icon/><strong>{label}</strong></li>)}</ol>
      <p className="contact-process-notice"><AlertTriangle/>문의 접수는 업무 수임을 의미하지 않으며, 상담 내용과 업무 범위를 확인한 후 진행 여부가 결정됩니다.</p>
    </div></section>
    <main className="section contact-main"><div className="shell contact-layout">
      <Suspense fallback={<div className="contact-form-loading">상담 신청서를 불러오는 중입니다.</div>}><ContactApplicationForm/></Suspense>
