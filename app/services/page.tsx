...
export const metadata: Metadata = publicMetadata({
  ...
  description: "기업 인허가·인증, 농업경영, 노인복지사업 창업, 행정심판·민원행정 등 가든 행정사사무소의 4대 핵심 업무를 안내합니다.",
  path: "/services",
});
...
const serviceFaqs = [
  ...
  ["전화나 온라인으로 상담할 수 있나요?", "평일 09:00~19:00에는 010-5834-5300으로 전화 상담을 요청할 수 있습니다. 온라인 접수는 저장 연동이 활성화된 경우에만 이용할 수 있습니다."],
...
export default function ServicesPage() {
  ...
  return <>
    ...
    <section className="services-hero"><div className="shell services-hero-inner">
      <div className="services-hero-copy"><p className="eyebrow">4 CORE SERVICES</p><h1>복잡한 행정업무,<br/><strong>4개 핵심 분야로 안내합니다.</strong></h1><p>기업 인허가·인증, 농업경영, 노인복지사업 창업, 행정심판·민원행정까지<br/>현재 상황을 확인하고 필요한 절차와 준비 방향을 안내합니다.</p><div className="button-row"><a className="button gold" href="#service-finder">내 업무 분야 찾기</a><Link className="button outline-light" href="/contact">상담 신청하기</Link></div></div>
    ...
    <section className="section service-cases"><div className="shell"><div className="services-heading split"><div><p className="eyebrow">ACTUAL WORK CASES</p><h2>주요 업무사례</h2><p>실제 의뢰 업무의 진행 결과를 분야별로 확인할 수 있습니다.</p></div><Link className="text-link" href="/cases">전체 사례 보기 <ArrowRight/></Link></div><div className="service-case-list">{caseExamples.slice(0, 3).map((example, index) => <article key={example.slug}><span>{String(index + 1).padStart(2, "0")}</span><div><small>업무사례 · {example.categoryLabel}</small><h3>{example.title}</h3><p>{example.summary}</p></div><Link href={`/cases/${example.slug}`}>사례 상세 보기 <ArrowRight/></Link></article>)}</div></div></section>
