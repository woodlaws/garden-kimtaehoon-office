...
function getValues(body: Record<string, unknown>): ContactFormValues {
  return {
    ...
    startupInterest: clean(body.startupInterest, 40),
    websitePackage: clean(body.websitePackage, 40),
    supportProgram: clean(body.supportProgram, 500),
...
export async function POST(request: Request) {
  ...
  const payload = {
    ...
    startupInterest: values.startupInterest,
    websitePackage: values.websitePackage,
    supportProgram: values.supportProgram,
    phone: values.phone,
    email: values.email,
    title: values.title,
