...
export type BlogPostPreview = Pick<BlogPost, "slug" | "category" | "title" | "summary" | "date" | "thumbnail">;

export function BlogFilter({ posts }: { posts: BlogPostPreview[] }) {
  ...
  return <>
    <div className="filter-tools"><label className="search-box"><Search/><span className="sr-only">블로그 검색</span><input value={query} onChange={e => setQuery(e.target.value)} placeholder="궁금한 행정정보를 검색하세요"/></label>
      <div className="chips" aria-label="카테고리 필터">{categories.map(c => <button key={c} onClick={() => setCategory(c)} className={c === category ? "selected" : ""} aria-pressed={c === category}>{c}</button>)}</div>
    </div>
    <div className="blog-grid">{filtered.map((post, i) => <article className="blog-card" key={post.slug}>{post.thumbnail ? <Link className="blog-photo-thumb" href={`/blog/${post.slug}`} aria-label={`${post.title} 읽기`}><Image src={post.thumbnail.src} alt={post.thumbnail.alt} width={post.thumbnail.width} height={post.thumbnail.height} sizes="(max-width: 600px) calc(100vw - 48px), (max-width: 900px) 50vw, 380px" /><span>{post.category}</span></Link> : <div className={`thumb thumb-${(i % 3) + 1}`}><span>{post.category}</span></div>}<div className="card-body"><p className="meta">{post.category} · {post.date}</p><h2><Link href={`/blog/${post.slug}`}>{post.title}</Link></h2><p>{post.summary}</p><Link className="text-link" href={`/blog/${post.slug}`}>글 읽기 →</Link></div></article>)}</div>
