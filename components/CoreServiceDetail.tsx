...
export function CoreServiceDetail({ service }: { service: CoreService }) {
  ...
  return <>
    ...
    <section className="section core-service-cases"><div className="shell">
      ...
      {service.slug === "agriculture-consulting" && <aside className="agriculture-startup-link"><Sprout/><div><p className="eyebrow">STARTUP MARKETING</p><h3>농장과 상품의 온라인 홍보·판매도 준비하고 계신가요?</h3><p>블로그·홈페이지·스마트스토어 중 현재 사업 단계에 맞는 채널과 제작 범위를 확인해 보세요.</p></div><Link className="button outline" href="/startup-marketing">창업마케팅 보기 <ArrowRight/></Link></aside>}
    </div></section>
